...
describe('<Typography />', () => {
  ...
  it('should render a h5', () => {
    render(<Typography variant="h5">{children}</Typography>);

    expect(screen.getByText(children).tagName).toBe('H5');
  });

  it('should render a h6', () => {
    render(<Typography variant="h6">{children}</Typography>);

    expect(screen.getByText(children).tagName).toBe('H6');
  });

  it('should render a chosen tag', () => {
    render(<Typography component="h1" variant="h6">{children}</Typography>);
  ...
  it('should work with a single value', () => {
    render(<Typography variant="h6" variantMapping={{ h6: 'aside' }}>{children}</Typography>);
